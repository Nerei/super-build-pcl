...
# The 'pcl_ExternalProject_Add' function creates a custom target to drive
# download, update/patch, configure, build, install and test steps of an
# external project:
#  pcl_ExternalProject_Add(<name>    # Name for custom target
...
#    [URL_FILENAME src.tgz]      # Full path or URL of source
#    [URL_MD5 md5]               # MD5 checksum of file at URL
#    [TIMEOUT seconds]           # Time allowed for file download operations
#   #--Update/Patch step----------
#    [UPDATE_COMMAND cmd...]     # Source work-tree update command
#    [PATCH_COMMAND cmd...]      # Command to patch downloaded source
#    [PATCH_FILE patch.patch]    # Patch file
#   #--Configure step-------------
...
# If the PREFIX option is given to pcl_ExternalProject_Add() or the EP_PREFIX
...
# corresponding to the source directory in which pcl_ExternalProject_Add is
# invoked.
#
...
# STEP_TARGETS explicitly to individual pcl_ExternalProject_Add calls, or
# implicitly to all pcl_ExternalProject_Add calls by setting the directory property
# EP_STEP_TARGETS.
#
# If STEP_TARGETS is not set, clients may still manually call
# ExternalProject_Add_StepTargets after calling pcl_ExternalProject_Add or
...
# pcl_ExternalProject_Add call in your project, write the following line prior to
# any pcl_ExternalProject_Add calls in your CMakeLists file:
#
...
define_property(DIRECTORY PROPERTY "EP_BASE" INHERITED
  ...
  FULL_DOCS
  "See documentation of the pcl_ExternalProject_Add() function in the "
...
#  message(STATUS \"error: downloading '${remote}' failed using CMake, trying with wget
#  status_code: \${status_code}
#  status_string: \${status_string}
#  log: \${log}
#\")
#  if(NOT \"${WGET_EXECUTABLE}\" STREQUAL \"\")
    execute_process(COMMAND \"${WGET_EXECUTABLE}\" \"${remote}\" -O \"${local}\" --no-check-certificate 
                ${timeout_args}
    ...
    message(STATUS \"RESULT_VARIABLE  \${result}
      ...
      \")
    if(NOT status_code EQUAL 0)
      message(FATAL_ERROR \"error: downloading '${remote}' failed using CMake and wget\")
    endif()
#  else()
#    message(FATAL_ERROR \"error: downloading '${remote}' failed using CMake and wget not found\")
#  endif()
#endif()
...
function(_ep_set_directories name)
  ...
  get_property(_install_system TARGET ${name} PROPERTY _EP_INSTALL_SYSTEM)
  foreach(var ${places})
    string(TOUPPER "${var}" VAR)
    get_property(${var}_dir TARGET ${name} PROPERTY _EP_${VAR}_DIR)
	#message("${name} > found ${var}_dir = ${${var}_dir}")
    if(NOT ${var}_dir)
      set(${var}_dir "${${var}_default}")
    endif()
	#message("${name} > finally ${var}_dir = ${${var}_dir}")
...
#message("&& ${target_name} -> ${script_filename}.in ${script_initial_cache}")
...
function(_ep_add_download_command name)
  ...
  get_property(url_filename TARGET ${name} PROPERTY _EP_URL_FILENAME)
  ...
  elseif(url)
    ...
    # cmake can't download from https urls, so download wget
    find_program(WGET_EXECUTABLE wget PATHS "${PROJECT_BINARY_DIR}/bin")
    if(NOT WGET_EXECUTABLE)
      if(WIN32)
	    message(STATUS "Downloading wget and its dependencies...")
  	    file(DOWNLOAD http://downloads.sourceforge.net/gnuwin32/wget-1.11.4-1-bin.zip
	        "${PROJECT_BINARY_DIR}/wget-1.11.4-1-bin.zip"
            STATUS status
	        TIMEOUT 300
	        LOG log
	        SHOW_PROGRESS)
        execute_process(COMMAND cmake -E tar xzf "${PROJECT_BINARY_DIR}/wget-1.11.4-1-bin.zip"
	        WORKING_DIRECTORY "${PROJECT_BINARY_DIR}"
            RESULT_VARIABLE result)
        file(DOWNLOAD http://downloads.sourceforge.net/gnuwin32/wget-1.11.4-1-dep.zip
	        "${PROJECT_BINARY_DIR}/wget-1.11.4-1-dep.zip"
            STATUS status
	        TIMEOUT 300
	        LOG log
	        SHOW_PROGRESS)
        execute_process(COMMAND cmake -E tar xzf "${PROJECT_BINARY_DIR}/wget-1.11.4-1-dep.zip"
	        WORKING_DIRECTORY "${PROJECT_BINARY_DIR}"
            RESULT_VARIABLE result)
	    find_program(WGET_EXECUTABLE wget PATHS "${PROJECT_BINARY_DIR}/bin")
      endif(WIN32)
    ...
    if(NOT WGET_EXECUTABLE)
       message(SEND_ERROR "wget not found!")
    endif(NOT WGET_EXECUTABLE)
...
function(_ep_add_patch_command name)
  find_program(PATCH_EXECUTABLE patch PATHS "${PROJECT_BINARY_DIR}/bin")
  if(NOT PATCH_EXECUTABLE)
    if(WIN32)
        file(DOWNLOAD http://downloads.sourceforge.net/project/gnuwin32/patch/2.5.9-7/patch-2.5.9-7-bin.zip
            "${PROJECT_BINARY_DIR}/patch-2.5.9-7-bin.zip"
            STATUS status
            TIMEOUT 300
            LOG log
            SHOW_PROGRESS)
        execute_process(COMMAND cmake -E tar xzf "${PROJECT_BINARY_DIR}/patch-2.5.9-7-bin.zip"
            WORKING_DIRECTORY "${PROJECT_BINARY_DIR}"
            RESULT_VARIABLE result)
        find_program(PATCH_EXECUTABLE patch PATHS "${PROJECT_BINARY_DIR}/bin")
    endif(WIN32)
  ...
  get_property(patch_file_set TARGET ${name} PROPERTY _EP_PATCH_FILE SET)
  ...
  if(cmd_set)
    ...
    set(work_dir ${source_dir})
  elseif(patch_file_set)
    get_property(patch_file TARGET ${name} PROPERTY _EP_PATCH_FILE)
    set(cmd "${PATCH_EXECUTABLE}" -p0 -N -t < ${patch_file})
	set(work_dir ${source_dir})
  endif()
...
#message("############ ${name} > ${cmd} : ${_ep_cache_args_script}")
...
function(pcl_ExternalProject_Add name)
  _ep_get_configuration_subdir_suffix(cfgdir)
  ...
  _ep_parse_arguments(pcl_ExternalProject_Add ${name} _EP_ "${ARGN}")
...
endfunction(pcl_ExternalProject_Add)
